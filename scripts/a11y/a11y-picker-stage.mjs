...
await page.goto("http://localhost:4999/press-and-slide-picker?embed=1&set=status");
...
await page.mouse.up();
await page.waitForTimeout(300);
// Forced placement: "up" on a top row still grows upward when it fits;
// "right" on a right-edge chip cannot fit, so auto takes over.
await page.evaluate(() => { document.querySelector(".picker-demo__scroll-area").scrollTop = 0; });
await page.selectOption("#demo-controls-set", "status");
await page.selectOption("#demo-controls-open", "up");
await page.waitForTimeout(200);
const fchip = page.locator(".psp-chip").nth(3);
const fb = await fchip.boundingBox();
await page.mouse.move(fb.x + fb.width / 2, fb.y + fb.height / 2);
await page.mouse.down();
await page.waitForTimeout(400);
const fstrip = await page.locator(".psp-strip").boundingBox();
push("forced 'up' grows upward from a mid row", (await page.locator(".psp-strip--vertical").count()) === 1 && fstrip.y + fstrip.height <= fb.y + 1, fstrip && { bottom: fstrip.y + fstrip.height, chipTop: fb.y });
await page.keyboard.press("Escape");
await page.mouse.up();
await page.waitForTimeout(300);
await page.selectOption("#demo-controls-open", "right");
await page.waitForTimeout(200);
await page.mouse.move(fb.x + fb.width / 2, fb.y + fb.height / 2);
await page.mouse.down();
await page.waitForTimeout(400);
const rstrip = await page.locator(".psp-strip").boundingBox();
push("forced 'right' with no room falls back to a fitting layout", rstrip !== null && rstrip.x >= 0 && rstrip.x + rstrip.width <= 390, rstrip);
await page.keyboard.press("Escape");
await page.mouse.up();
await page.waitForTimeout(300);
