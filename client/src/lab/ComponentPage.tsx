...
export function ComponentPage({ component }: { component: LabComponent }) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      {!chromeHidden && (
        <div className="z-40 shrink-0 border-b border-[var(--border-subtle)] bg-[var(--surface-overlay)] backdrop-blur-md">
          <div className="mx-auto flex w-full max-w-6xl flex-wrap items-center justify-between gap-x-4 gap-y-2 px-4 py-2.5 sm:px-6">
            ...
            <div
              className="flex shrink-0 items-center gap-0.5 rounded-full border border-[var(--border-subtle)] bg-[var(--bg-canvas)] p-0.5"
              role="tablist"
        ...
        {tab === "usage" && (
          <div className="h-full overflow-auto">
            <div className="mx-auto w-full max-w-3xl px-4 py-8 sm:px-6">
              ...
              <h3 className="mt-8 text-sm font-semibold uppercase tracking-wide text-[var(--text-tertiary)]">
                Try it
              </h3>
              <ul className="mt-3 space-y-2">
                {component.tryIt.map(hint => (
                  <li
                    key={hint}
                    className="flex gap-2.5 text-[0.9375rem] text-[var(--text-secondary)]"
                  >
                    <span
                      aria-hidden="true"
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {component.dependencies.map(dep => (
                  <span
                    key={dep}
                    className="rounded-full border border-[#e4e0ea] bg-white px-3 py-1 font-mono text-xs text-[var(--text-secondary)]"
                  >
                    {dep}
                  </span>
                ))}
              ...
              <p className="mt-8 pb-8 text-sm text-[var(--text-tertiary)]">
